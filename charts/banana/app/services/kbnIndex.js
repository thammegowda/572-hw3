  ...
  module.service('kbnIndex', function($http, alertSrv) {
    ...
    function all_indices() {
      var something = $http({
        ...
        url: config.solr + config.solr_core + "/select?q=*:*&wt=json&rows=0&omitHeader=true&facet=true&facet.range=dates&facet.range.start=NOW-10YEARS/DAY&facet.range.end=NOW&facet.range.gap=%2B1DAY&facet.mincount=1",
      ...
      return something.then(function(p) {
        ...
        var timestamp_array = p.data.facet_counts.facet_ranges.dates.counts;
